...
from lib.stackit import ServerEnricher, run_stackit_query  # noqa: E402

log = get_logger("stackit.servers")


def collect_servers(project_id: str) -> list[dict]:
    """Listet die Server eines Projekts und reichert jeden Eintrag an."""
    enricher = ServerEnricher()
    servers = run_stackit_query(["server", "list", "--project-id", project_id])
    return [enricher.enrich(s, project_id) for s in servers]


def _cell(value: object) -> str:
    """Formatiert einen Zellwert für die Tabelle; None/leer wird zu '-'."""
    return str(value) if value not in (None, "") else "-"


def _os_label(row: dict) -> str:
    """Kompaktes OS-Label, z. B. 'ubuntu 22.04' oder 'windows'."""
    distro_version = " ".join(p for p in (row["os_distro"], row["os_version"]) if p)
    return distro_version or row["os"] or "-"


def render_table(rows: list[dict]) -> None:
    header = (
        f"{'Name':22} {'Power':9} {'OS':18} {'vCPU':>4} {'RAM(GB)':>7} "
    ...
    for r in rows:
        print(
            f"{r['name']:22.22} {r['power_status']:9.9} {_os_label(r):18.18} "
            f"{_cell(r['vcpus']):>4} {_cell(r['ram_gb']):>7} "
            f"{_cell(r['private_ips']):16.16} {_cell(r['public_ips']):16.16} {r['machine_type']}"
        )
